...
void CutID::CutBasedID(int reg,double EFFAr[5][3]){
  //  TFile *f = new TFile("/afs/hep.wisc.edu/home/sacharya/photonID/GJets/CMSSW_10_2_18/src/gJets/merged/ALLPV/PF/isopt/Isopt/MC_GJets_HT40-100_350.root");
  // TTree *t1 = (TTree*)f->Get("t1");

  TFile *f1;
    if(reg == 1) f1 =  new TFile("CutTMVABarrelTrainner_TruePV_1_60.root","recreate");
  if(reg == 2) f1 =  new TFile("CutTMVAEndCapTrainner_TruePV_1_60.root","recreate");
 

  /*  if(reg == 1) f1 =  new TFile("CutTMVABarrelUL1.root","recreate");
  if(reg == 2) f1 =  new TFile("CutTMVABarrelUL2.root","recreate");
  if(reg == 3) f1 =  new TFile("CutTMVABarrelUL3.root","recreate");
  if(reg == 4) f1 =  new TFile("CutTMVAEndCapUL1.root","recreate");
  if(reg == 5) f1 =  new TFile("CutTMVAEndCapUL2.root","recreate");
  */
